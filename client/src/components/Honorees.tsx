...
}> = ({ name, title, description, trait1, trait2, icon1, icon2, imagePlaceholder }) => {
  return (
    <div className="bg-background text-foreground p-8 rounded-sm shadow-lg border-2 border-secondary transform transition-transform hover:scale-105 duration-300">
      <div className="relative mb-6 overflow-hidden rounded-sm" style={{ paddingTop: "100%" }}>
      ...
      <div className="text-center">
        ...
        <div className="flex justify-center space-x-4 text-primary">
          <span className="inline-flex items-center"><i className={`${icon1} mr-2`}></i> {trait1}</span>
          <span className="inline-flex items-center"><i className={`${icon2} mr-2`}></i> {trait2}</span>
        </div>
      </div>
    </div>
  );
...
const Honorees: React.FC = () => {
  ...
  const backgroundStyle = {
    backgroundImage: "url('https://images.unsplash.com/photo-1552084117-56a987666449?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80')",
    backgroundSize: "cover",
    backgroundPosition: "center"
  };

  return (
    <section id="honorees" className="py-16 md:py-24 px-4 bg-primary text-primary-foreground relative">
